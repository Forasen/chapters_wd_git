...
public:
    ...
    public:
        ...
        //charProxy(const String& str, int index);不能传递const类型

        //返回当前字符
        operator char() const;//类型转换函数尽量不要使用！
    };

    friend charProxy;//声明为又远类，
...
String::charProxy String::operator[](int index)
{
 ...
 const String::charProxy String::(int index) const
 {
...
//=========String::charProxy============
String::charProxy(String& str, int index)
    ...
    if(charIndex_<len && charIndex_>=0)
    {
       if(--(theString_.pstr_[len+1]) != 0)
        {//多个对象
            ...
            theString.pstr_[len+1] = 1;
        }
        else
        {//单一对象
            theString_.pstr_[len+1] = 1;
            theString_.pstr_[len] = ch;
        }
        return *this;
    }
    else
